...
import { POST } from "@/app/api/lobby/[id]/set-map/route";

describe("POST /api/lobby/[id]/set-map", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/lobby/lobby-1/set-map", {
        method: "POST",
        body: JSON.stringify({ map_id: "map-1" }),
      }),
      params,
    );
  ...
  it("returns 403 when user is not the leader", async () => {
    ...
    const body = await response.json();
    expect(body.error).toBe("Only the lobby leader can set the map");
  });

  it("returns 400 when lobby is not in playing phase", async () => {
    mockSupabaseClient.auth.getUser.mockResolvedValue({
      data: { user: { id: "user-1" } },
      error: null,
    });
    ...
    const body = await response.json();
    expect(body.error).toBe("Lobby must be in playing phase");
  });
  ...
  it("returns 404 when map not found", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/lobby/lobby-1/set-map", {
        method: "POST",
        body: JSON.stringify({ map_id: "map-unknown" }),
      }),
      params,
    );
  ...
  it("returns 200 and sets map successfully", async () => {
    mockSupabaseClient.auth.getUser.mockResolvedValue({
      data: { user: { id: "user-1" } },
      error: null,
    });
    ...
    const mapQuery = {
      select: vi.fn(() => mapQuery),
      eq: vi.fn(() => mapQuery),
      single: vi.fn(() => Promise.resolve({ data: { id: "map-1" }, error: null })),
    };
